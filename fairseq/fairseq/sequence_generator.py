...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        # ===== < !!! transduce decode !!!> ===== #
        """
        [20220228] Sadly, not working well.
        Note:
        1. The code below only support batch size 1.
        
        Observations:
        1. Pad is not a zero vector in the original GSLM (weird).
        2. To predict the next token, it highly depends on the previous predicted tokens.
        Therefore, when the window on the src side moves, 
        if we keep the previous predicted tokens in the input, it's very like the LM will predict eos.
        
        Conclusion: 
        1. the simpler way is to just segment the input unit (audio).
        

        def transduce(cond, step, transduce_state):
            r = []
            sep = 0
            pad = 1
            eos = 5
            ...
            # find sep token
            seps = (tokens == sep).nonzero().tolist()
            idxs = [s[0] for s in seps]
            sep_is = [s[1] for s in seps]

            # transduce
            for i in range(cond.shape[0]):
                if i in idxs:

                    sep_i = sep_is[idxs.index(i)]

                    # check if model has satisfied output all the tokens with the src_window
                    t = transduce_state[i][0]
                    if int(cond[i][-1]) == eos and not transduce_state[i][1]:
                        t = t + 1
                        transduce_state[i][0] = t

                    # src window (src_start: src_end)
                    src_start = 1 + window * t
                    if 1 + window * (t + 1) >= sep_i:
                        # has read all the src
                        src_end = sep_i
                        transduce_state[i][1] = True
                    else:
                        src_end = 1 + window * (t + 1)

                    src_window = torch.clone(cond[i][src_start:src_end])

                    # tgt window
                    if len((cond[i] == 5).nonzero().tolist()) > 0:
                        tgt_start = (cond[i] == 5).nonzero().tolist()[-1][0]
                    else:
                        tgt_start = sep_i + 1
                    tgt_window = torch.clone(cond[i][tgt_start:])
                    if not transduce_state[i][1]:
                        tgt_window = tgt_window[tgt_window != 5]

                    c = torch.cat(
                        (
                            torch.tensor(2).unsqueeze(0).to(src_window),
                            src_window,
                            torch.tensor(sep).unsqueeze(0).to(src_window),
                            tgt_window,
                        )
                    )
                    r.append(c)
            ...
            # === collate (narrow and stack)=== #
            return torch.stack(r), transduce_state

        # ===== < !!! transduce state !!!> ===== #
        # (timestep, if_read_all_the_src)
        # transduce_state = [[0, False] for _ in range(bsz)]
        # ====================================== #
        """
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            """
            Legacy code for trying transducer. SAD SAD...
            # cond = torch.clone(tokens[:, : step + 1])
            # if len((cond == 0).nonzero().tolist()) > 0 and len(cond[0]) > 700:
            #     print("wait")
            # t_state = [transduce_state[i] for i in original_batch_idxs.tolist()]
            # cond, t_state = transduce(cond, step, t_state)
            """
            # ====== <!!! forward decoder !!!> ===== #
