...
def cli_main(modify_parser: Optional[Callable[[argparse.ArgumentParser], None]] = None) -> None:
    ...
    # === metadata === #
    parser.add_argument("--date", type=str, default="00000000")

    # === prompt args === #
    parser.add_argument("--prompt_task", type=str, default="IC")
    parser.add_argument("--prefix_prompt_length", type=int, default=6)
    parser.add_argument("--infix_prompt_length", type=int, default=0)
    ...
    if args.fine_tune:
        args.prefix_prompt_length = 0
        args.infix_prompt_length = 0
    # ===================== #
    ...
    input_args = [
        ...
        "--seed=100501",
        "--fp16",
        f"--tensorboard-logdir=./logs/{prompt_task}_{unit_model}_log_{date}",
        # ==== prompt ==== #
        "--arch=transformer_lm_big_prompt",
        "--criterion=cross_entropy_prompt",
        "--user-dir=./prompt_lm_module",
        f"--restore-file=./pretrained_models/{unit_model}_lm/checkpoint_best.pt",
        f"--save-dir=checkpoints/{prompt_task}_{unit_model}_checkpoints_{date}",
        f"--prefix_prompt_length={prefix_prompt_length}",
        f"--infix_prompt_length={infix_prompt_length}",
        "--use_sep_token",
        "--deep_prompt",
    ]
    # fine-tune the whole model
    if args.fine_tune:
        input_args.append("--fine_tune")
        input_args.append("--lr-scheduler=inverse_sqrt")
        input_args.append("--dropout=0.1")
        input_args.append("--attention-dropout=0.1")
        input_args.append("--save-interval=5")
        input_args.append("--lr=5e-4")
    # prompting
    else:
        input_args.append("--dropout=0")
        input_args.append("--attention-dropout=0")
        input_args.append("--save-interval=5")
        input_args.append("--lr=5e-3")

    COLOR = "\033[92m"
    ENDC = "\033[0m"
    print(f"{COLOR}[INFO] Input args: {input_args}{ENDC}")
